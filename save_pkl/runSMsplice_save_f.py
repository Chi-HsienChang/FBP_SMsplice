...
# time python runSMsplice_save_f.py -c ./canonical_datasets/lengthfilter_canonical_dataset_dm6.txt -a ./allSS_datasets/allSS_dataset_dm6.txt -g ./fly.fa -m ./maxEnt_models/fly/ --print_predictions --learning_seed real-decoy --max_learned_weights 15 --max_learned_scores 1000 --learn_sre
# time python runSMsplice_save_f.py -c ./canonical_datasets/canonical_dataset_dm6_original.txt -a ./allSS_datasets/allSS_dataset_dm6.txt -g ./fly.fa -m ./maxEnt_models/fly/ --print_predictions --learning_seed real-decoy --max_learned_weights 15 --max_learned_scores 1000 --learn_sre
...
for gene in canonical.index:
    txstart = canonical.loc[gene, 4] - 1
    txend = canonical.loc[gene, 5]
    # For dm6, chromosome names are like "2L", "X", "3L", etc.
    chrom = canonical.loc[gene, 2]  
    # Now use the whole chromosome string (do not slice [3:])

    if chrom not in genome:
        print(f"⚠️ Warning: Chromosome {chrom} not found in genome.")
        continue  # 或 raise/skip 處理

    region = genome[chrom][canonical.loc[gene,4]-1:canonical.loc[gene,5]]
    if not np.array_equiv(['a', 'c', 'g', 't'], np.unique(region.seq.lower())):
        # print(gene, "nonstandard base")
        continue
    ...
    description = (gene + ' GeneID:' + gene +
                   ...
                   ' Chromosome:' + canonical.loc[gene, 2] +
                   ' Start:' + str(txstart) +
                   ' Stop:' + str(txend) +
                   ' Strand:' + canonical.loc[gene, 3])

    if canonical.loc[gene, 3] == '-':
        seq = region.seq.reverse_complement()
    elif canonical.loc[gene, 3] == '+': 
        seq = region.seq
    else:
        print(gene, "strand error")
...
lengthIntronsDF = pd.DataFrame(list(zip(np.arange(0, N), lengthIntronsHist)), columns = ['Index', 'Count'])
lengthIntronsDF['Prob'] = lengthIntronsDF['Count']/lengthIntronsDF['Count'].sum()
pIL = geometric_smooth_tailed(lengthIntrons, N, 5, 2000, lower_cutoff=40)
...
print(f"len(testGenes): {len(testGenes)}")
...
with open(f'f_pred_all_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)
...
with open(f'f_predictions_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)
...
# 將字典儲存到 pickle 檔案
with open(f'f_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)
