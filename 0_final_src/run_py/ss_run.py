...
for i in range(17, 1629):  # 循环遍历指定索引范围
    # 打印当前正在处理的索引，输出到原始标准输出（终端）
    original_stdout.write(f"Running index {i}...\n")

    if lengths[i] <= 3000 or lengths[i] > 5000:
        print(f"sequence is too long ({lengths[i]}).")
        continue
    
    fold_name = f"./{my_seed}_h_result_all_parse/h_result_5000"
    output_filename = f"./{my_seed}_h_result_all_parse/h_result_5000/000_human_g_{i}.txt"  # 注意这里用 i 作为文件名的一部分，避免重复写入同一个文件
    import os
