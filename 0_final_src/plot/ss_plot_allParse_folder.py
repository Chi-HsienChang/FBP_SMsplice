...
# 檢查命令列參數：dataset 與 max_length
if len(sys.argv) < 3:
    print("Usage: script.py <dataset> <max_length>")
    sys.exit(1)
...
def get_files_for_topk(dataset, max_length):
    """
    根據 dataset 和 top_k 值，返回匹配的文件列表
    """
    if dataset == "t":
        pattern = f"./{my_seed}_t_result_all_parse/t_result_{max_length}/000_arabidopsis_g_*.txt"
    elif dataset == "z":
        pattern = f"./{my_seed}_z_result_all_parse/z_result_{max_length}/000_zebrafish_g_*.txt"
    elif dataset == "m":
        pattern = f"./{my_seed}_m_result_all_parse/m_result_{max_length}/000_mouth_g_*.txt"
    elif dataset == "h":
        pattern = f"./{my_seed}_h_result_all_parse/h_result_{max_length}/000_human_g_*.txt"
    elif dataset == "f":
        pattern = f"./{my_seed}_f_result_all_parse/f_result_{max_length}/000_fly_g_*.txt"
    elif dataset == "o":
        pattern = f"./{my_seed}_o_result_all_parse/o_result_{max_length}/000_moth_g_*.txt"
...
def get_bin_info(files, threshold):
    ...
    # 定義區間，包含 [-1,0) 區間，但頂區間改為 "[0.9, 1.0]"
    bin_edges = np.array([-1.0, 0.0] + list(np.arange(0.1, 0.9, 0.1)) + [0.9, 1.0])
    bin_labels = [
...
def save_bin_info_to_csv(dataset, max_length_val, threshold):
    """
    將指定 threshold 下的 bin 統計資訊存入 CSV 檔
    """
    files = get_files_for_topk(dataset, max_length_val)
    bin_info_df = get_bin_info(files, threshold)
    outcsv = f"../{my_seed}_{dataset}_top_{max_length_val}.csv"
...
def plot_all_species_pies(max_length_val, threshold=0.00):
    # 構造各 species 資料
    species_data = {}
    for code, full_name in species_names.items():
        files = get_files_for_topk(code, max_length_val)
    ...
    plt.savefig(f"v_in_fbp_pie_top_{max_length}_seed_{my_seed}.png")
    print(f"Pie and bar charts saved to precision_distribution_by_bins_{max_length}.png")


def main():
    # 若 dataset 參數不是 "all"，則只處理該單一物種並存出 CSV
    if dataset_arg != "all":
        files = get_files_for_topk(dataset_arg, max_length)
        bin_info_df = get_bin_info(files, 0.00)
        print("Bin information for predictions with prob >= 0.00:")
        print(bin_info_df)
        save_bin_info_to_csv(dataset_arg, max_length, 0.00)
    else:
        # 若指定 "all"，則針對所有物種處理，並各自存出 CSV
        for code in species_names.keys():
            bin_info_df = get_bin_info(get_files_for_topk(code, max_length), 0.00)
            save_bin_info_to_csv(code, max_length, 0.00)
    # 繪製所有物種圖形：每個 species 左側圓餅圖、右側直方圖
    # plot_all_species_pies(max_length, 0.00)
