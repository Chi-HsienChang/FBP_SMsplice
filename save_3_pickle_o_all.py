...
# time python save_3_pickle_o_all.py -c ./canonical_datasets/canonical_dataset_Bmori_2016v1.0_original.txt -a ./allSS_datasets/allSS_dataset_Bmori_2016v1.0.txt -g ./moth.fa -m ./maxEnt_models/moth/ --prelearned_sres moth --print_predictions
...
for gene in canonical.index:
    try:
        txstart = int(canonical.loc[gene, 4]) - 1
        txend = int(canonical.loc[gene, 5])
    except Exception as e:
        print(f"Error converting txstart/txend for gene {gene}: {e}")
        continue

    # Read the chromosome string directly
    chrom = str(canonical.loc[gene, 2]).strip()

    # Filter out mitochondrial genes
    if "mitochondrial" in chrom.lower():
        continue

    # First try the exact key
    if chrom not in genome:
        # Try removing the trailing version number if present
        chrom_alt = chrom.split('.')[0]
        if chrom_alt in genome:
            chrom = chrom_alt
        else:
            print(f"Chromosome {chrom} not found in genome for gene {gene}.")
            continue

    # Get the genomic region and check if it contains only a, c, g, t
    region_seq = genome[chrom][txstart:txend].seq.lower()
    # Compare sorted unique bases with ['a','c','g','t']
    if not np.array_equiv(np.sort(np.unique(list(region_seq))), np.array(['a','c','g','t'])):
        # Uncomment next line to print a warning:
        # print(gene, "nonstandard base")
        continue

    name = gene
    geneID = gene
    description = (gene + ' GeneID:' + gene +
                   ' TranscriptID:Canonical' +
                   ' Chromosome:' + str(canonical.loc[gene, 2]) +
                   ' Start:' + str(txstart) +
                   ' Stop:' + str(txend) +
                   ' Strand:' + str(canonical.loc[gene, 3]))
    ...
    if strand == '-':
        ...
        print(f"{gene} strand error: {strand}")
        continue

    genes[gene] = SeqRecord(seq, name=name, id=geneID, description=description)
    
    # Assume column 6 contains a comma-separated list of exon start positions.
    # exon_starts_str = str(canonical.loc[gene, 6]).strip()
    # if exon_starts_str and exon_starts_str != ',':
    #     exon_starts = [x for x in exon_starts_str.split(",") if x]
    #     numExonsPerGene.append(len(exon_starts))
    # else:
    #     numExonsPerGene.append(0)

# if len(genes) == 0:
#     raise ValueError("No genes were loaded. Check chromosome name matching between your canonical file and the FASTA file.")

# print("Loaded", len(genes), "genes.")

# # -----------------------------------------------------
# # Compute N from number of exons (avoid empty list error)
# if numExonsPerGene:
...
testGenes = canonical[(canonical[1] == 0)&canonical[2].isin(['BHWX01000012.1', 'BHWX01000013.1', 'BHWX01000018.1', 'BHWX01000021.1', 'BHWX01000022.1', 'BHWX01000027.1', 'BHWX01000038.1', 'BHWX01000074.1', 'BHWX01000097.1'])].index 
# testGenes = canonical[(canonical[1] == 0)&canonical[2].isin(['2L', '3L'])].index 
...
lengthIntronsDF = pd.DataFrame(list(zip(np.arange(0, N), lengthIntronsHist)), columns = ['Index', 'Count'])
lengthIntronsDF['Prob'] = lengthIntronsDF['Count']/lengthIntronsDF['Count'].sum()
pIL = geometric_smooth_tailed(lengthIntrons, N, 15, 3000, lower_cutoff=60)

lengthFirstExonsHist = np.histogram(lengthFirstExons, bins = N, range = (0,N))[0] # nth bin is [n-1,n) bun Nth bin is [N-1,N]
lengthFirstExonsDF = pd.DataFrame(list(zip(np.arange(0, N), lengthFirstExonsHist)), columns = ['Index', 'Count'])
lengthFirstExonsDF['Prob'] = lengthFirstExonsDF['Count']/lengthFirstExonsDF['Count'].sum()
# pELF = geometric_smooth_tailed(lengthFirstExons, N, 30, 300, lower_cutoff=0)
pELF = adaptive_kde_tailed(lengthFirstExons, N, 0.80, 0)

lengthMiddleExonsHist = np.histogram(lengthMiddleExons, bins = N, range = (0,N))[0] # nth bin is [n-1,n) bun Nth bin is [N-1,N]
lengthMiddleExonsDF = pd.DataFrame(list(zip(np.arange(0, N), lengthMiddleExonsHist)), columns = ['Index', 'Count'])
lengthMiddleExonsDF['Prob'] = lengthMiddleExonsDF['Count']/lengthMiddleExonsDF['Count'].sum()
# pELM = geometric_smooth_tailed(lengthMiddleExons, N, 30, 500, lower_cutoff=0)
pELM = adaptive_kde_tailed(lengthMiddleExons, N, 0.80, 0)

lengthLastExonsHist = np.histogram(lengthLastExons, bins = N, range = (0,N))[0] # nth bin is [n-1,n) bun Nth bin is [N-1,N]
lengthLastExonsDF = pd.DataFrame(list(zip(np.arange(0, N), lengthLastExonsHist)), columns = ['Index', 'Count'])
lengthLastExonsDF['Prob'] = lengthLastExonsDF['Count']/lengthLastExonsDF['Count'].sum()
# pELL = geometric_smooth_tailed(lengthLastExons, N, 100, 750, lower_cutoff=0)
pELL = adaptive_kde_tailed(lengthLastExons, N, 0.85, 0)
pELS = 0*np.copy(pELL)
...
with open(f'o_pred_all_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)
...
with open(f'o_predictions_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)
...
# 將字典儲存到 pickle 檔案
with open(f'o_new_{my_seed}.pkl', 'wb') as f:
    pickle.dump(data, f)


print("testGenes len = ", len(testGenes))
