...
# 1. 讀入 CSV 並處理表頭
# ======================
file_path = "H.csv"  # <-- 修改為你的 CSV 路徑/檔名
...
# 自動找出 bin label 欄位（包含 "Unnamed: 1"），並將其重新命名為 "Human"
bin_label_col = [col for col in clean_df.columns if "Human" in col][0]
clean_df.rename(columns={bin_label_col: "Human"}, inplace=True)
bin_labels = clean_df["Human"]
...
def plot_species_all_k(species_label):
    ...
    # bar_ax.legend(handles=legend_handles,bbox_to_anchor=(0.87, 0.99)) # <-- For Human
