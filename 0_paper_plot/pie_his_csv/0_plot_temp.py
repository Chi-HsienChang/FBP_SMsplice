...
# 1. 讀入 CSV 並處理表頭
# ======================
file_path = "A.csv"  # <-- 修改為你的 CSV 路徑/檔名
...
# 自動找出 bin label 欄位（包含 "Unnamed: 1"），並將其重新命名為 "Arabidopsis"
bin_label_col = [col for col in clean_df.columns if "Arabidopsis" in col][0]
clean_df.rename(columns={bin_label_col: "Arabidopsis"}, inplace=True)
bin_labels = clean_df["Arabidopsis"]
...
def plot_species_all_k(species_label):
    ...
    for i, k in enumerate(["100", "500", "1000"]):
        ...
        axs[i].set_position([pos.x0, pos.y0, pos.width, pos.height])
        
        # Legend 放在圖形正下方
        axs[i].legend(wedges, legend_labels, loc="lower center", bbox_to_anchor=(0.5, -0.49),
                      fontsize=13, ncol=2)
        axs[i].set_title(f"Arabidopsis\n$k={k}$", fontsize=20)
    ...
    bar_ax.set_xticklabels(bins, rotation=45, ha="right", fontsize=13)
    bar_ax.set_ylim(0, 100)
    bar_ax.set_title(f"Precision by Bin", fontsize=20)
    bar_ax.set_ylabel("Precision", fontsize=13)
    ...
    # bar_ax.legend(handles=legend_handles,bbox_to_anchor=(0.87, 0.99)) # <-- For Arabidopsis
    
    # fig.suptitle(f"{species_label} - Score Distribution and Precision", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(f"{species_label}_high.png", dpi=300, bbox_inches='tight')
    plt.show()

# ======================
# 測試繪圖（單一物種）
# ======================
plot_species_all_k("Arabidopsis")
