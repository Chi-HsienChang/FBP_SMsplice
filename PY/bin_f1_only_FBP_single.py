...
def plot_threshold_metrics(dataset, topk_list, thresholds):
    ...
    with open(outtxt, "w") as f:
        for topk, metrics in all_metrics.items():
            ...
            f.write(f"    Recall   : {metrics['Viterbi']['Recall']:.8f}\n")
            f.write(f"    Precision: {metrics['Viterbi']['Precision']:.8f}\n")
            f.write(f"    F1       : {metrics['Viterbi']['F1']:.8f}\n")
            f.write("  Threshold Metrics:\n")
            for thr, m in metrics["Thresholds"].items():
                f.write(f"    Threshold {thr:.2f} -> Recall: {m['Recall']:.8f}, Precision: {m['Precision']:.8f}, F1: {100*m['F1']:.8f}%\n")
            f.write("\n")
