...
def save_results_to_txt(dataset, topk_list, thresholds):
    ...
    with open(outtxt, "w") as f:
        for top_k_val in topk_list:
            ...
            f.write(f"  Recall: {viterbi_metrics[0]:.8f}, Precision: {viterbi_metrics[1]:.8f}, F1: {viterbi_metrics[2]:.8f}\n")
            f.write("Threshold Metrics:\n")
            for thr in thresholds:
                thr_metrics = evaluate_all_files_threshold(files, thr)
                f.write(f"  Threshold >= {thr:.2f}: Recall: {thr_metrics[0]:.8f}, Precision: {thr_metrics[1]:.8f}, F1: {thr_metrics[2]:.8f}\n")
            f.write("\n")
